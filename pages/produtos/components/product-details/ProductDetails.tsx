  ...
  return (
    <div className="border rounded-lg p-3 space-y-2.5 text-sm">
      <div className="flex items-start gap-2">
        <Icon icon="fa-store" className="text-amber-600 text-xs mt-0.5 flex-shrink-0" />
        <div className="flex-1 min-w-0">
          <span className="text-zinc-600">Fornecido por </span>
          <Link
            href={getStoreUrl(store)}
            className="font-semibold text-zinc-900 hover:text-amber-700 transition"
          >
            {store?.title}
          </Link>
        </div>
      ...
      {product?.assembly === "on" && (
        <div className="flex items-start gap-2">
          <Icon icon="fa-tools" className="text-amber-600 text-xs mt-0.5 flex-shrink-0" />
      ...
      {productColors.length > 0 && (
        <div className="flex items-start gap-2">
          <Icon icon="fa-paint-brush" className="text-amber-600 text-xs mt-0.5 flex-shrink-0" />
          <div className="flex-1 min-w-0">
      ...
      {productCategories.length > 0 && (
        <div className="flex items-start gap-2">
          <Icon icon="fa-tags" className="text-amber-600 text-xs mt-0.5 flex-shrink-0" />
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap gap-1.5">
              {productCategories.map((child: RelationType) => (
                <Link
                  ...
                  className="bg-amber-50 hover:bg-amber-100 text-amber-700 text-xs py-1 px-2.5 rounded-full transition"
                >
                  {child.title}
                </Link>
              ))}
      ...
      {productTags.length > 0 && (
        <div className="flex items-start gap-2">
          <Icon icon="fa-bookmark" className="text-amber-600 text-xs mt-0.5 flex-shrink-0" />
          <div className="flex-1 min-w-0">
