...
export default function Filter(attrs: FilterPainelType) {
  ...
  const filterRelatedGroups = (clickedElement: Element) => {
    if (
      !clickedElement.element_related_id ||
      clickedElement.element_related_id.length === 0
    ) {
      return;
    }
    ...
    allGroups.forEach((group) => {
      const shouldDisplay = group.elements.some(
        (el) =>
          relatedElementIds.includes(el.id) && !activeElements.includes(el.id)
      );
  ...
  const removeRelatedGroups = (clickedElement: Element) => {
    ...
    const updatedDisplayedGroups = displayedGroups.filter((group) => {
      const hasActiveElementInGroup = group.elements.some(
        (el) => activeElements.includes(el.id) && el.id !== clickedElement.id
      );
      const isOriginalFirstGroup = allGroups[0]?.id === group.id;
      const isRelatedToDeselected = group.elements.some((el) =>
        relatedElementIds?.includes(el.id)
      );
      const isCurrentlyRelatedToActive = allGroups.some(
        (g) =>
          g.id === group.id &&
          g.elements.some(
            (e) =>
              activeElements.includes(e.id) &&
              e.element_related_id?.includes(clickedElement.id)
          )
      );

      return (
        hasActiveElementInGroup ||
        isOriginalFirstGroup ||
        !isRelatedToDeselected ||
        isCurrentlyRelatedToActive
      );
    });
