...
export default function PartnersStep({ partners, onChange, onAdd, onRemove }: Props) {
  return (
    <div className="space-y-6">
      {partners.map((p, i) => (
        <div key={`partner-${i}`} className="border rounded-lg p-4 space-y-3">
          ...
          <div className="grid md:grid-cols-2 gap-3">
            ...
            <Input name={`p-email-${i}`} placeholder="E-mail" value={p.email ?? ""} onChange={(e) => onChange(i, "email", e.target.value)} />
          </div>
          ...
          <Input name={`p-job-${i}`} placeholder="Profissão ou cargo" value={p.professional_occupation ?? ""} onChange={(e) => onChange(i, "professional_occupation", e.target.value)} />
      ...
      <Button style="btn-light" type="button" onClick={onAdd}>Adicionar outro sócio</Button>
    </div>
  );
}
