  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4">
        <Input name="email" placeholder="E-mail principal" value={data.email} onChange={(e) => onChange("email", e.target.value)} required />
      ...
      {isPJ ? (
        <div className="grid gap-4">
          ...
          <Input name="annual_revenue" type="number" placeholder="Faturamento anual" value={data.annual_revenue ?? ""} onChange={(e) => onChange("annual_revenue", e.target.value ? Number(e.target.value) : null)} required />
        </div>
      ) : (
        <div className="grid gap-4">
          <Input name="birth_date" type="date" placeholder="Data de nascimento" value={data.birth_date ?? ""} onChange={(e) => onChange("birth_date", e.target.value)} required />
      ...
      <Input name="name" placeholder={isPJ ? "Nome de quem responde pela loja" : "Nome completo"} value={data.name} onChange={(e) => onChange("name", e.target.value)} required />
      <Input name="professional_occupation" placeholder={isPJ ? "Cargo na empresa" : "Profissão"} value={data.professional_occupation ?? ""} onChange={(e) => onChange("professional_occupation", e.target.value)} />
    </div>
  );
}
